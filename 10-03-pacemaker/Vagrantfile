...
virt_machines=[
  {
	...
	:ip_private_hb => "192.168.2.10",
  },
  {
    :hostname => "nodetwo",
    :ip_public => "192.168.0.202",
    :ip_private => "192.168.1.20",
	:ip_private_hb => "192.168.2.20"
  }
]
...
Vagrant.configure("2") do |config|
	virt_machines.each do |machine|
		...
		config.vm.define machine[:hostname] do |node|
			...
			node.vm.network :private_network, ip: machine[:ip_private_hb]
			node.vm.provider "virtualbox" do |current_vm, override|
				...
				override.vm.provision "shell", path: HOST_CONFIG_SCRIPT, args: ['test', HOST_USER_PASS, HOST_UPGRADE, machine[:hostname], machine[:ip_public], machine[:ip_private], machine[:ip_private_hb]], run: "once"
			end
		end
	end
end