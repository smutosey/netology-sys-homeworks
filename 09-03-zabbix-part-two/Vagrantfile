...
virt_machines=[
  {
    :hostname => "nedorezov-zabbixagent1",
    :ip => "192.168.0.161"
  },
  {
    :hostname => "nedorezov-zabbixagent2",
    :ip => "192.168.0.162"
  }
]
...
ZABBIX_SERVER_IP = '192.168.0.106'
	...
	virt_machines.each do |machine|
		...
		config.vm.define machine[:hostname] do |node|
			...
			node.vm.provider "virtualbox" do |current_vm, override|
				...
				override.vm.provision "shell", path: 'zabbix-agent.sh', args: [	'test', 	'123456789',	'false', 	machine[:hostname], 	machine[:ip],	ZABBIX_SERVER_IP], run: "once"
			end
		end
	end
end